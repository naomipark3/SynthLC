...
S2_STAY_AT_LEAST_TWO: assume property (@(posedge clk_i) first |-> s_eventually
    ( !mult_fsm_s5 [*0:$] ##1 mult_fsm_s5 ##1 mult_fsm_s5));

// if precondition in unreachable meaning s2 already left when s1 enters
`ifndef WHB
HB_wb_stage_s1_LEAVING_mult_fsm_s5: assert property (@(posedge clk_i) 
    ($past(!wb_stage_s1_hpn) && wb_stage_s1 && (mult_fsm_s5_hpn || mult_fsm_s5)) |=>
     mult_fsm_s5);
`else 
WHB_wb_stage_s1_LEAVING_mult_fsm_s5: assert property (@(posedge clk_i) 
    ($past(!wb_stage_s1_hpn) && wb_stage_s1 && mult_fsm_s5_hpn) |->
    mult_fsm_s5);
WHB_CONCUR_wb_stage_s1_LEAVING_mult_fsm_s5: assert property (@(posedge clk_i) 
    ($past(!wb_stage_s1_hpn) && wb_stage_s1 && mult_fsm_s5_hpn && mult_fsm_s5) |=>
    (!mult_fsm_s5));
`endif 
