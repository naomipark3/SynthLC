...
S2_STAY_AT_LEAST_TWO: assume property (@(posedge clk_i) first |-> s_eventually
    ( !div_fsm_s9 [*0:$] ##1 div_fsm_s9 ##1 div_fsm_s9));

// if precondition in unreachable meaning s2 already left when s1 enters
`ifndef WHB
HB_wb_stage_s1_LEAVING_div_fsm_s9: assert property (@(posedge clk_i) 
    ($past(!wb_stage_s1_hpn) && wb_stage_s1 && (div_fsm_s9_hpn || div_fsm_s9)) |=>
     div_fsm_s9);
`else 
WHB_wb_stage_s1_LEAVING_div_fsm_s9: assert property (@(posedge clk_i) 
    ($past(!wb_stage_s1_hpn) && wb_stage_s1 && div_fsm_s9_hpn) |->
    div_fsm_s9);
WHB_CONCUR_wb_stage_s1_LEAVING_div_fsm_s9: assert property (@(posedge clk_i) 
    ($past(!wb_stage_s1_hpn) && wb_stage_s1 && div_fsm_s9_hpn && div_fsm_s9) |=>
    (!div_fsm_s9));
`endif 
